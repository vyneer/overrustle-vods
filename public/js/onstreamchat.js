var Chat = function() {
	this.videoId = getUrlParameter("id");
	console.log(this.videoId);
	this.status = "loading";
	this.chatDelay = 0;
	tSubstring = getUrlParameter("t").substring(0, getUrlParameter("t").length - 1);
	time = tSubstring.replace(/([hm])/g, ":")
	this.previousTimeOffset = moment.duration(time).asSeconds()
	...
	window.setInterval(function() {
		if (self.status == "running" && self.chat) {
			var currentTimeOffset = self.previousTimeOffset + 1;
